...
**Repo layout note:** the frontend (React/TypeScript, built with Lovable)
lives at this repo's root. All backend code -- the actual agent
pipeline, evaluation harness, and submission documentation -- is under
[`backend/`](backend/). If you're here to evaluate the hackathon
...
This repo contains both the frontend (built with Lovable) and the
backend (this project's actual submission focus -- the agent pipeline,
evaluation, and documentation). The frontend lives at the repo root
(`src/`, `package.json`, etc.); all backend code and docs are under
`backend/`.

```
decision-debate-agent/
  src/, public/, package.json, ...   Frontend (React/TypeScript, built with Lovable)
  FRONTEND.md                          Original frontend-specific README
  backend/
    agents/
    ...
    requirements.txt
    .env.example
    README.md               Copy of this file, for convenience when browsing backend/ directly
    REPRODUCTION.md
    TRAJECTORIES.md
```

**All backend commands in [`backend/REPRODUCTION.md`](backend/REPRODUCTION.md)
are run from inside the `backend/` folder** (`cd backend` first). See that
file for exact setup and run instructions from a clean environment, and
[`backend/TRAJECTORIES.md`](backend/TRAJECTORIES.md) for representative
agent trajectories (both the solution's own agents and the coding agent
